...
mark.group <- function (comps,supp) {
  ...
  if(!supp.ordered[1] & sum(supp.ordered[seq(supp.ordered) > 1]) == 1) #If the 'All' category is not suppressed and there is exactly one suppressed subpart, then suppress all the individual subparts.
...
stats.table[!Suppressed & Marked,]
stats.table[DistrictId=='0101000' &  SchoolYearId==4 & ScopeId==1 & SchoolId=='All' & SubjectCode == 'All' & Subgroup %in% c('NELL', 'ELL') & GradeEnrolled== 'All',]
